export const GET_PROJECT_LIST_SUCCESS = 'GET_PROJECT_LIST_SUCCESS';
export const GET_PROJECT_LIST_FAIL = 'GET_PROJECT_LIST_FAIL';

export const GET_PROJECT_BY_ID_SUCCESS = 'GET_CLIENT_BY_ID_SUCCESS';
export const GET_PROJECT_BY_ID_FAIL = 'GET_CLIENT_BY_ID_FAIL';

export const GET_PROJECTS_BY_STATUS_SUCCESS = 'GET_PROJECTS_BY_STATUS_SUCCESS';
export const GET_PROJECTS_BY_STATUS_FAIL = 'GET_PROJECTS_BY_STATUS_FAIL';

export const GET_CLIENT_LIST_SUCCESS = 'GET_CLIENT_LIST_SUCCESS';
export const GET_CLIENT_LIST_FAIL = 'GET_CLIENT_LIST_FAIL';

export const GET_CLIENT_BY_ID_SUCCESS = 'GET_CLIENT_BY_ID_SUCCESS';
export const GET_CLIENT_BY_ID_FAIL = 'GET_CLIENT_BY_ID_FAIL';